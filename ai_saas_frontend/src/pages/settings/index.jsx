...
export default function Settings() {
  ...
  const translatePlanName = (planName) => {
    if (!planName) return t("common.not_informed");
          ...
          {user ? (
            <>
              <p className="text-gray-700 text-sm">
                <strong className="font-semibold text-gray-900 text-sm">{t("settings.plan_modal.current_plan")}</strong> {translatePlanName(user.plan?.name)}
              </p>
          ...
          {user ? (
            <>
              ...
              <p className="text-gray-700 text-sm">
                ...
                {new Date(user.created_at).toLocaleDateString(t("dates.locale"))}
              </p>
            </>
          ) : (
            <p>{t("common.loading")}</p>
