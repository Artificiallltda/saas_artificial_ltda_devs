  ...
  const [messages, setMessages] = useState([
    {
      id: 1,
      role: 'assistant',
      content: t('generation.video.assistant.greeting')
    }
  ]);
    ...
    try {
      ...
      if (res?.video?.id) {
        ...
        const assistantMessage = {
          id: Date.now() + 1,
          role: 'assistant',
          content: t('generation.video.success_message'),
          video: videoUrl
        };
      ...
      const errorMessage = {
        id: Date.now() + 1,
        role: 'assistant',
        content: t('generation.video.error_message')
      };
      setMessages(prev => [...prev, errorMessage]);
    } finally {
      setLoading(false);
                    ...
                    >
                      {VIDEO_RATIOS.map((r) => (
                        <option key={r.value} value={r.value}>
                          {t(`generation.video.ratios.${r.value === '16:9' ? 'landscape' : 'portrait'}`)}
                        </option>
                      ))}
                    </select>
                  </div>
